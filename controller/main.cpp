...
//auto prob_len = labels_input[0].second->text().toInt();
//auto m = labels_input[1].second->text().toInt();
//auto exp_count = labels_input[2].second->text().toInt();
//auto p_vals = labels_input[3].second->text().toInt();
//auto deviation = labels_input[3].second->text().toDouble();
//auto model = ChenModel(Vector<double>::generate_normal(prob_len), m);
//auto b = model.calc_p_values(exp_count, p_vals, model.deviate(deviation)).count_buckets(20);
...
int main(int argc, char *argv[]) {
    ...
    QMainWindow window;
    auto chartView = new TableGraphWidget(
            Config{"Chen",
                   {"bucket"},
                   {"count"},
                   {
                           ...
                           {"exp_n", 150},
                           {"p_val_n", 1000},
                           {"deviation", 0.025, Config::Field::DOUBLE},
                   }
            },
            &window
    );

//    auto chartView = new GraphWidget(&window);
